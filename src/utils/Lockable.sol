...
abstract contract Lockable is Ownable {
    // Timestamp until which the user is locked (0 means unlocked)
    mapping(address => uint256) private _lockupUntil;

    // Iterable tracking for locked addresses (may include expired entries until pruned)
    address[] private _lockedAccounts;

    // 1-based index in _lockedAccounts (0 means not present)
     ...
     * @dev Reverts on zero address or past timestamps. Emits {Lock}.
     */
    function lockUntil(address who, uint256 until) external onlyOwner {
     ...
     * @dev Reverts on zero address or zero duration. Emits {Lock}.
     */
    function lockFor(address who, uint256 duration) external onlyOwner {
     ...
     * @dev Emits {Unlock}. No-op if already unlocked.
     */
     ...
     */
    function lockupUntil(address who) public view returns (uint256) {
        return _lockupUntil[who];
    }

    /**
     * @dev Internal helper to determine current lock state.
     * @param who The address to query.
     * @return True if now < lock expiry; false otherwise.
     */
    ...
    /**
     * @notice Returns the length of the internal locked list (may include expired until pruned).
     * @return The number of entries in the list.
     */
    function lockedCount() public view returns (uint256) {
     ...
     */
    function lockedAt(uint256 index) public view returns (address who, uint256 until) {
        who = _lockedAccounts[index];
        until = _lockupUntil[who];
    }

    /**
     * @notice Returns whether an address is present in the locked list (may be expired).
     * @param who The address to check.
     * @return True if present in the list; false otherwise.
     */
    function isInLockedList(address who) public view returns (bool) {
     ...
     * @dev Returns up to `limit` active locks starting from `offset`. The returned arrays
     *      are sized to `limit`, but only the first `count` entries are valid.
     */
     ...
     */
    function pruneExpired(uint256 max) external onlyOwner returns (uint256 pruned) {
